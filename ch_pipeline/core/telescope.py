...
class CHIME(telescope.PolarisedTelescope):
    ...
    def __init__(self, feeds=None):
        ...
        # Create an empty dictionary to cache products
        # required for fast evaluation of the primary beam.
        self._beam_pre_products = {}
    ...
    def beam(self, feed, freq, angpos=None):
        ...
        # Calculate the products necessary to compute the beam and cache them
        if freq not in self._beam_pre_products:

            self._beam_pre_products[freq] = cylbeam.fast_beam_pol_precompute(
                self.zenith,
        ...
        # Evaluate the beam using pre-computed products
        beam = cylbeam.fast_beam_pol_eval(
            angpos, self._beam_pre_products[freq], pol, use_horizon=True
        )
