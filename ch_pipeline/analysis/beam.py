...
class TransitGrouper(task.SingleTask):
    ...
    def setup(self, observer=None):
        ...
        try:
            ...
            msg = (
                f"Could not find source {self.source} in catalogue. "
    ...
    def _finalize_transit(self):
        ...
        if len(self.tstreams) == 0 or self.cur_transit is None:
            ...
            return None
        self.log.debug(
            f"Finalising transit for {ephem.unix_to_datetime(self.cur_transit)}..."
        )
    ...
    def _transit_bounds(self):
        ...
        if len(this_run) == 0:
            self.log.warning(
                f"Could not find source transit in holography database for {ephem.unix_to_datetime(self.cur_transit)}."
            )
            # skip this file
            self.cur_transit = None
        else:
...
class FilterHolographyProcessed(task.MPILoggedTask):
    ...
    def next(self, intervals):
        ...
        for fi in intervals:
            ...
            if len(this_obs) == 0:
                self.log.warning(
                    f"Could not find source transit in holography database for {ephem.unix_to_datetime(start)}."
                )
            elif this_obs[0].id in self.proc_transits:
                self.log.warning(
                    f"Already processed transit for {ephem.unix_to_datetime(start)}. Skipping."
                )
            else:
                files += fi[0]
