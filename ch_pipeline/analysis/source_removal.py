...
def solve_multiple_times(vis, weight, source_model, rcond=None):
    ...
    # Solve for model coefficients
    coeff = np.linalg.lstsq(C, np.dot(S.T.conj(), weight * vis), rcond=rcond)[0]
            ...
            for ff, nu in enumerate(freq):
                ...
                if self.extended_source_kwargs:
                    ...
                    while iters < self.max_iter:
                        ...
                        coeff = solve_multiple_times(
                            vis, weight, model, rcond=self.rcond
                        )
    ...
    def process(self, data, beams):
        ...
        for pp, upp in enumerate(upol):
            ...
            for ff, nu in enumerate(freq):
                ...
                out_coeff[ff, pp, :] = solve_multiple_times(
                    vis, weight, source_model, rcond=self.rcond
                )
