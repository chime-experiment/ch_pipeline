...
class ThermalCalibration(task.SingleTask):
    ...
    def process(self, data):
        ...
        any_nans = np.any(np.isnan(gain.gain[:]), axis=0)

        if any_nans.all():
            raise RuntimeError(f"Could not get gains - entire dataset is {np.nan}")
        ...
        if any_nans.any():
            ...
            # Use the standard gain value where gains are undetermined
            gain.gain[:, any_nans] = 1.0
